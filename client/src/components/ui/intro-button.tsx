...
const IntroButton = ({ onClick, title, limit, paddingB, className, disabled }) => {
  const DisabledButton = () => (
    ...
    >
      ...
      <p className="text-white text-sm font-light relative z-50 transition-all duration-800">{title}</p>
    </div>
  )
  return (
    !disabled ? (    <button
    ...
    >
      <div className="blur-container backdrop-blur-4"></div>
      <p className="text-white text-sm font-light relative z-50 translate-x-3 group-hover:-translate-x-1 transition-all duration-800">{title}</p>
